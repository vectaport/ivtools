...
| import.comt     | import()                                                               | reading a drawtool-format file from disk (not the in-process export(:string)/run(:str) round-trip gsexim.comt covers) -- a recognized top-level object imports promptly, and an unrecognized one fails the read and returns rather than hanging |
| frontback.comt  | front() back() delete() rect()                                         | front()/back() reject a wrong-typed argument and a view whose comp is already gone (deleted) instead of appending it to the Clipboard, returning nil in both cases; a live graphic still works normally afterward |
| carousel.comt | trans() front() raster() mod() carousel.next()                            | integration coverage for the `examples/carousel.comt` coverflow demo -- front slot widest and centered with side slots shrinking symmetrically, `carousel.next()` rotates the deck and every slot converges back to the same target geometry regardless of which photo now occupies it, and repeated advances wrap the photo library instead of erroring |
