...
| carousel.comt | trans() front() raster() mod()                                            | integration coverage for the `examples/carousel.comt`/`carousel_next.comt` coverflow demo -- front slot widest and centered with side slots shrinking symmetrically, one advance rotates the deck and every slot converges back to the same target geometry regardless of which photo now occupies it, and repeated advances wrap the photo library instead of erroring |
