...
void ComKit::Init (OverlayComp* comp, const char* name) {
    OverlayKit::Init(comp, name);
}

MenuItem * ComKit::MakeFileMenu() {
...
MenuItem * ComKit::MakeToolsMenu() {
    ...
    menu_item->action(new ActionCallback(ComKit)(this, &ComKit::toolbar1));
    mbi->menu()->append_item(menu_item);

    menu_item = kit.menu_item(kit.label("Idraw Tools"));
    menu_item->action(new ActionCallback(ComKit)(this, &ComKit::toolbar0));
    mbi->menu()->append_item(menu_item);

    return mbi;
}

Glyph* ComKit::MakeToolbar() {
    WidgetKit& kit = *WidgetKit::instance();
