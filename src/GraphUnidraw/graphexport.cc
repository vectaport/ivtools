...
ClassId GraphExportCmd::GetClassId () { return GRAPH_EXPORT_CMD; }

boolean GraphExportCmd::IsA (ClassId id) {
    return GRAPH_EXPORT_CMD == id || OvExportCmd::IsA(id);
}

GraphExportCmd::GraphExportCmd (ControlInfo* c, ExportChooser* f) : OvExportCmd(c) { 
    Init(f); 
}

GraphExportCmd::GraphExportCmd (Editor* ed, ExportChooser* f) : OvExportCmd(ed) { 
    Init(f); 
}

GraphExportCmd::~GraphExportCmd () { 
}
...
boolean GraphExportCmd::dot_format() { return strcmp("dot", chooser_->format())==0; }
