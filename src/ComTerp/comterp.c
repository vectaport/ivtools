...
void ComTerp::eval_expr_internals(int pedepth) {
  ...
  if (sv.type() == ComValue::CommandType) {
	...
	if (has_streams)
	  break;   // any stream arg triggers the overdrive
      }
    if (has_streams) {
      AttributeValueList* avl = new AttributeValueList();
...
void ComTerp::add_defaults() {
  ...
  if (!_defaults_added) {
    ...
    add_command("next_parent", new NextParentFunc(this), nil, nil, true /* hidden: prototype/test-only, see NextParentFunc's docstring */);
