...
void Fig31::add_point(Coord x, Coord y) {
    if (_count == 0) {
        _xmin = x - 1;
        _xmax = x + 1;
        _ymin = y - 1;
...
void Fig31::request(Requisition& requisition) const {
    if (_count > 0) {
...
void Fig31::allocate(Canvas* c, const Allocation& a, Extension& ext) {
    if (_count > 0) {
...
void Fig31::draw(Canvas* c, const Allocation& allocation) const {
    if (c != nil && _count > 0) {
...
) : Fig31(brush, stroke, fill, false, false, 2) {
    add_point(x1, y1);
    add_point(x2, y2);
}
...
Fig31Rectangle::Fig31Rectangle (
    const Brush* brush, const Color* stroke, const Color* fill,
    Coord l, Coord b, Coord r, Coord t
) : Fig31(brush, stroke, fill, true, false, 4) {
    add_point(l, b);
    add_point(l, t);
    add_point(r, t);
    add_point(r, b);
...
Fig31Open_BSpline::Fig31Open_BSpline (
    const Brush* brush, const Color* stroke, const Color* fill,
    Coord* x, Coord* y, int n
) : Fig31(brush, stroke, fill, false, true, (n + 2) * 3 + 1) {
...
Fig31Closed_BSpline::Fig31Closed_BSpline (
    const Brush* brush, const Color* stroke, const Color* fill,
    Coord* x, Coord* y, int n
) : Fig31(brush, stroke, fill, true, true, n * 3 + 1) {
...
Fig31Polyline::Fig31Polyline (
    const Brush* brush, const Color* stroke, const Color* fill,
    Coord* x, Coord* y, int n
) : Fig31(brush, stroke, fill, false, false, n) {
    add_point(x[0], y[0]);
    for (int i = 1; i < n; ++i) {
        add_point(x[i], y[i]);
    }
...
Fig31Polygon::Fig31Polygon (
    const Brush* brush, const Color* stroke, const Color* fill,
    Coord* x, Coord* y, int n
) : Fig31(brush, stroke, fill, true, false, n) {
    add_point(x[0], y[0]);
    for (int i = 1; i < n; ++i) {
        add_point(x[i], y[i]);
    }
