...
public:
    ...
    virtual void execute();
    virtual const char* docstring() {
      return "name=%s([:shiftarrow flag] [:reset]) -- portable NAME of the next unread keystroke over the canvas (nil if none); each press returned once.  A bare modifier keypress (Shift/Ctrl/CapsLock/Alt/Meta/Super/Hyper alone) is never returned -- its effect shows up as uppercasing on whatever key comes next instead.  Any key with a standard C character literal comes back as that literal -- letters/digits as themselves, \"\\x1b\" esc, \" \" space, \"\\r\" enter, \"\\t\" tab, \"\\b\" backspace, \"\\x7f\" delete.  Keys with no character form get a name, cloned from Python's curses KEY_* constants: \"up\" \"down\" \"left\" \"right\" \"home\" \"end\" \"ppage\" \"npage\" \"ins\" \"F1\"..\"F12\"; anything else unmapped falls back to the decimal keysym.  If shift was held or caps lock was on, arrows/home/end/ppage/npage/ins come back UPPERCASE (\"UP\", \"HOME\") -- for letters this already falls out of the keysym itself; there's no separate prefix, just case.  Function keys are always \"F1\"..\"F12\" regardless of shift -- no established shifted-F-key convention exists, and on hardware where an F-key requires holding fn (current Mac keyboards) the combination barely arises.  :shiftarrow true additionally captures modified arrows AND letters -- Shift held OR Caps Lock on -- suppressing their pan/tool-shortcut, until :shiftarrow false, :reset, or the poll watchdog lapses ~2s"; }
    virtual const char** dockeys() {
      static const char* keys[] = {
