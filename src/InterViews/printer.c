...
static void do_color(ostream& out, const Color* color) {
  ...
  if(a == 1.0) {
    out << (double)r << " " << (double)g << " " << (double)b << " setrgbcolor\n";
  }
  else {
    float c = (r + g + b)/3.0;
    if(c < 0.5)  out  << (double)(1.0 - a) << " " << "setgray\n";
    else out  << (double)a << " " << "setgray\n";
  }
}
...
static void do_brush(ostream& out, const Brush* brush) {
    ...
    out << (double)linewidth << " setlinewidth\n";
...
void Printer::page(const char* label) {
    ...
    *p->out_ << (double)-(p->x_) << " " << (double)-(p->y_) << " translate\n";
...
void Printer::transform(const Transformer& t) {
    ...
    *p->out_ << "[" << (double)a00 << " " << (double)a01;
    *p->out_ << " " << (double)a10 << " " << (double)a11;
    *p->out_ << " " << (double)a20 << " " << (double)a21 << "] concat\n";
}
...
void Printer::move_to(Coord x, Coord y) {
    ostream& out = *rep_->out_;
    flush();
    out << (double)x << " " << (double)y << " moveto\n";
}

void Printer::line_to(Coord x, Coord y) {
    ostream& out = *rep_->out_;
    flush();
    out << (double)x << " " << (double)y << " lineto\n";
}
...
void Printer::curve_to(
    ...
    ostream& out = *rep_->out_;
    flush();
    out << (double)x1 << " " << (double)y1 << " " << (double)x2 << " " << (double)y2 << " ";
    out << (double)x << " " << (double)y << " curveto\n";
}
...
void Printer::flush() {
    ...
    if (p->text_chars_ > 0) {
        out << ") ";
        if (p->text_spaces_ > 0) {
            out << p->text_spaces_ << " " << (double)p->text_curx_ << " ws\n";
        } else {
            out << p->text_chars_ << " " << (double)p->text_curx_ << " as\n";
        }
        p->text_chars_ = 0;
        p->text_spaces_ = 0;
    }
...
void Printer::image(const Raster* raster, Coord x, Coord y) {
    ...
    out << "gsave\n";
    out << "/picstr " << (double)width << " string def\n";
    out << (double)left << " " << (double)bottom << "  translate\n";
    out << (double)right - left << " " << (double)top - bottom << " scale\n";
    out << (double)width << " " << (double)height << " 8\n";
    out << "[" << (double)width << " 0 0 " << (double)height << " 0 0]\n";
