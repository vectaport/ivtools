...
OL_Specs::OL_Specs (Style* style) : Resource(), points_(12) {
    ...
    char fontname[20];
    snprintf(fontname, sizeof(fontname), "olglyph-%ld", points_);
...
OL_Mover::OL_Mover(
    ...
    if (font_ != nil) {
	FontBoundingBox box;
	font_->char_bbox((int) box_ul_, box);
	width_ = box.width() + 1;
...
void OL_Mover::draw(Canvas* c, const Allocation& a) const {
	...
	if (state_->test(TelltaleState::is_active)) {
	    c->character(font_, (int) fill_, width_, bg2, l, t);
	    c->character(font_, (int) box_ul_, width_, bg3, l, t);
	    c->character(font_, (int) box_lr_, width_, white, l, t);
	} else {
	    c->character(font_, (int) fill_, width_, bg1, l, t);
	    c->character(font_, (int) box_ul_, width_, white, l, t);
	    c->character(font_, (int) box_lr_, width_, bg3, l, t);
	}
...
void OL_Stepper::start_stepping() {
    if (initial_delay_ > 10) {
        Dispatcher::instance().startTimer(0, (int) initial_delay_, timer_);
    }
}

void OL_Stepper::next_step() {
    Dispatcher::instance().startTimer(0, (int) interval_, timer_);
}
...
void OL_Dragbox::draw(Canvas* c, const Allocation& a) const {
    ...
    const Font* f = s.font();
    if (f != nil) {
	c->character(f, (int) ul, width, upper_left, l, t);
	c->character(f, (int) box, width, fill, l, t);
	c->character(f, (int) lr, width, lower_right, l, t);
    }
}
