...
static char *
CheckMalloc(TIFF * tif, int n, char * what)
{
	char *cp = malloc(n);
	if (cp == NULL)
...
static
int EstimateStripByteCounts(TIFF * tif, TIFFDirEntry * dir, u_int dircount)
{
	register TIFFDirEntry *dp;
...
static
int MissingRequired(TIFF * tif, char * tagname)
{
	TIFFError(tif->tif_name,
 ...
 */
static
int CheckDirCount(TIFF * tif, TIFFDirEntry * dir, u_long count)
{
 ...
 */
static
int TIFFFetchData(TIFF * tif, TIFFDirEntry * dir, char * cp)
{
	int cc, w;
 ...
 */
static
int TIFFFetchString(TIFF * tif, TIFFDirEntry * dir, char * cp)
{
	if ((dir->tdir_count <= 4)) {
 ...
 */
static
int TIFFFetchNormalTag(TIFF * tif, TIFFDirEntry * dp)
{
 ...
 */
static
int TIFFFetchPerSampleShorts(TIFF * tif, TIFFDirEntry * dir, long * pl)
{
	u_short v[4];
 ...
 */
static
int TIFFFetchStripThing(TIFF * tif, TIFFDirEntry * dir, long nstrips, u_long ** lpp)
{
	register u_long *lp;
	int status;
...
#ifdef COLORIMETRY_SUPPORT
static
int TIFFFetchRefBlackWhite(TIFF * tif, TIFFDirEntry * dir)
{
 ...
 */
static
int TIFFFetchJPEGQTables(TIFF * tif, TIFFDirEntry * dir)
{
 ...
 */
static
int TIFFFetchJPEGCTables(TIFF * tif, TIFFDirEntry * dir, u_char *** ptab)
{
	long off[4];
	int i, j, ncodes;
	TIFFDirEntry tdir;
 ...
 */
static
int TIFFFetchExtraSamples(TIFF * tif, TIFFDirEntry * dp)
{
	int type;
